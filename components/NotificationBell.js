...
export default function NotificationBell() {
  ...
  return (
    <>
      ...
      {/* Dropdown Menu */}
      {isOpen && (
        <div className="absolute right-[-40] mt-3 w-80 md:w-96 bg-[#121212] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden ring-1 ring-white/5 animate-in fade-in zoom-in-95 duration-200">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      <p className={`text-sm truncate pr-2 ${!noti.isRead ? 'text-white font-bold' : 'text-gray-400'}`}>
                        {noti.movieName}
                      </p>
                      {!noti.isRead && <span className="w-2 h-2 bg-primary rounded-full flex-shrink-0 mt-1.5"></span>}
                    </div>
                    
                    <p className="text-primary text-xs font-medium mt-0.5">{noti.message}</p>
